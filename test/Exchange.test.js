...
contract('Exchange', ([deployer, feeAccount, user1, user2]) => {
  ...
  describe('making orders', async () => {
    ...
    it('tracks the newly created order', async () => {
      const ordersCount = await exchange.ordersCount();
      ordersCount.toString().should.equal('1');
      const order = await exchange.orders(ordersCount);
      order.id.toString().should.equal(ordersCount.toString(), 'id is correct');
      order.user.toString().should.equal(user1, 'user is correct');
      order.tokenGet.should.equal(token.address, 'tokenGet is correct');
      order.amountGet.toString().should.equal(tokens(1).toString(), 'amountGet is correct');
      order.tokenGive.should.equal(ETHER_ADDRESS, 'tokenGive is correct');
      order.amountGive.toString().should.equal(ether(1).toString(), 'amountGive is correct');
      order.timestamp.toString().length.should.be.at.least(1, 'id is correct');
    })
    ...
    it('emits an "Order" event', async () => {
      ...
      event.timestamp.toString().length.should.be.at.least(1, 'id is correct');
    })
  })
  ...
  describe('order actions', async () => {
    ...
    describe('filling orders', async () => {
      ...
      describe('failure', async () => {
        ...
        it('rejects allready-filled orders', async () => {
